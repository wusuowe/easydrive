...
query = "select code,`type`,content from Source where code='600000'"
...
for code,typ,content in cursor.fetchall():
    ...
    print dateList
    print len(dateList),len(updateList)
    for i in range(len(updateList)):
        ...
        updateStatement = "update figures set %s where code='%s' and rdate='%s';"%(updateList[i],code,dateList[i])
        print initStatement
